...
from agent_sarsa.conf.conf import Config
...
class Agent(BaseAgent):
    ...
    def exploit(self, env_obs):
        """
        Exploit current policy for evaluation with small exploration
        利用当前策略进行评估（包含小幅探索）
        ...
        act_data = ActData(act=self._epsilon_greedy(state=state, epsilon=0.1))
